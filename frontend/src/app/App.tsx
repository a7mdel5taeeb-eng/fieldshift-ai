...
export function App() {
  ...
  return <main className="app-shell"><section className="app-container">
    ...
    {step === 3 && <section className="results-panel"><div><p className="section-kicker">{t("steps.resultsKicker")}</p><h2>{t("results.heading")}</h2><p>{t("results.description")}</p></div>{analysisWarnings.length > 0 && <ul className="alert" aria-label="Analysis warnings">{analysisWarnings.map((warning) => <li key={warning}>{warning}</li>)}</ul>}<div className="result-grid">{fieldCards.map(([title, status, description]) => <article className="result-card" key={title}><p>{title}</p><h3 className={`status ${status.toLowerCase()}`}>{statusText(status)}</h3><span>{description}</span></article>)}</div><div className="results-columns"><article className="result-detail"><h3>{t("resultDetails.waterEvidence")}</h3>{moistureItems.length ? <ul>{moistureItems.map((item) => <li key={String(item.layer)}>{String(item.layer)}: {String(item.value)} {String(item.unit)} — {String(item.timestamp)}; {String(item.spatial_resolution)}; {String(item.source)}</li>)}</ul> : <p>{t("resultDetails.waterEmpty")}</p>}<p>{t("resultDetails.waterLimitation")}</p></article><article className="result-detail"><h3>{t("resultDetails.factorResults")}</h3>{factorItems?.length ? <ul>{factorItems.map((item) => <li key={String(item.factor)}><strong>{String(item.factor)}: {String(item.status)}</strong>{item.limitation ? ` — ${String(item.limitation)}` : ""}</li>)}</ul> : <p>{t("results.whyEmpty")}</p>}</article><article className="result-detail"><h3>{t("results.rotationOptions")}</h3><p>{scenarios ? t("results.rotationAvailable") : t("results.rotationEmpty")}</p></article><article className="result-detail"><h3>{t("results.why")}</h3><p>{suitability ? cropExplanation : t("results.whyEmpty")}</p></article></div><details className="evidence-details"><summary>{t("results.evidence")}</summary><EvidencePanel evidence={evidence} limitations={[t("evidence.missingCalendar"), t("evidence.smap"), t("evidence.preference"), t("evidence.compatibility")]} /><pre>{JSON.stringify({ environment, moisture, suitability, scenarios }, null, 2)}</pre></details></section>}
